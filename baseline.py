...
from transformers import BertTokenizer, BertForSequenceClassification, AdamW, BertConfig, get_linear_schedule_with_warmup
from transformers import XLNetTokenizer, XLNetModel, XLNetForSequenceClassification, RobertaTokenizer, RobertaForSequenceClassification, AlbertTokenizer, AlbertForSequenceClassification, AdamWeightDecay
...
df = pd.read_csv("data/train_eda.tsv", delimiter='\t', header=None, names=['id', 'label', 'alpha', 'sentence'])
dev_df = pd.read_csv("data/dev_real.tsv", delimiter='\t', header=None, names=['id', 'label', 'alpha', 'sentence'])

# Load the dataset into a pandas dataframe.
test_df = pd.read_csv("data/test_real.tsv", delimiter='\t', header=None, names=['id', 'sentence', 'label'])
sub = pd.read_csv('data/sample_submission.csv', dtype='str')
...
for epoch_i in range(0, epochs):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        loss = loss_function(outputs[1], b_labels)
    ...
    flat_valid = [item for sublist in valid for item in sublist]
    flat_valid = np.argmax(flat_valid, axis=1).flatten()

    # Combine the correct labels for each batch into a single list.
    flat_true_labels = [item for sublist in valid_true_labels for item in sublist]
           
    evaluate(flat_true_labels, flat_valid)
...
results['y_true'] = flat_true_labels
results['y_pred'] = flat_predictions

sub['Gold'] = flat_predictions

sub.to_csv('pred_eda.csv', index = 0)
